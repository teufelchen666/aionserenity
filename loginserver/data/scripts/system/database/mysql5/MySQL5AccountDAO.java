...
public class MySQL5AccountDAO extends AccountDAO {
     ...
     */
    @Override
    public int getPlayerAccess(String playerName, String hashPass)
    {
        int accessLevel = -1;
        PreparedStatement st = DB
            .prepareStatement("SELECT access_level FROM players P, account_data AD WHERE P.account_name = AD.name AND P.name = ? AND AD.password = ?");
